...
struct ReleasesDiscoveryView: View {
    @Environment(AppRouter.self) private var router
    @State private var viewModel = ReleasesDiscoveryViewModel()
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $showingResults) {
            ReleaseResultsView(viewModel: viewModel)
        }
    }
    ...
    private var emptyStateView: some View {
        ContentUnavailableView {
            ...
            Text("Search above to add artists you want to find releases for.")
        }
    }

    private var findReleasesButton: some View {
        Button {
            Task {
                await viewModel.findReleases()
                if viewModel.releasesError == nil {
                    showingResults = true
                }
            }
        } label: {
            ...
            .padding()
            .background(viewModel.canFindReleases ? Color.accentColor : Color.gray)
            .foregroundStyle(.white)
            .clipShape(RoundedRectangle(cornerRadius: 12))
        }
        .disabled(!viewModel.canFindReleases || viewModel.isLoadingReleases)
    }
}
